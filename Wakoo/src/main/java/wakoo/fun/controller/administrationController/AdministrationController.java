...
public class AdministrationController {
    ...
    public MsgVo administratorsCanQueryMultipleCriteria(HttpServletRequest request, String name, String nickname, String username, String roleName, String email, String mobile, String status, Integer pageNumber, Integer pageSize) {
        ...
        List<AdminAdministraltion> adminAdministraltions = adminAdministrationService.multiConditionQuery(name, nickname, username, roleName, email, mobile, status, (Integer) userId);
        // 从请求中获取名为userId的属性，并将其转换为Integer类型，赋值给userId1变量
        PageInfo<AdminAdministraltion> pageInfo = new PageInfo<>(list(request, 0, adminAdministraltions));
        pageInfo.setPageSize(pageSize);
        return new MsgVo(200, "查询成功", pageInfo);
    }

    public List<AdminAdministraltion> list(HttpServletRequest request, int parentId, List<AdminAdministraltion> list) {
        ...
        for (AdminAdministraltion a : list) {
            // 如果parentId等于1或者等于3
    ...
    public MsgVo administration(String keyword, Integer pageSize, Integer pageNumber, HttpServletRequest request) {
        ...
        // 调用adminAdministrationService的getsTheIdOfTheRole方法，传入userId1参数，获取一个角色ID，并赋值给role变量
        PageInfo<AdminAdministraltion> pageInfo = new PageInfo<>(list(request, 0, allAdministration));
